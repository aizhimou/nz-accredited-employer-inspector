...
describe("open-data snapshots", () => {
  it("discovers original files from the R2 bucket and updates the catalog", async () => {
    const scheduledTime = Date.parse("2041-01-05T16:15:00.000Z");
    const snapshotDate = "2041-01-06";
    const keys = [
      "catalog.json",
      `snapshots/${snapshotDate}/employers.csv`,
      `snapshots/${snapshotDate}/metadata.json`,
      "original/2025-06-02/List of Accredited Employers as at 2 June 2025.xlsx",
      "original/2026-07-27/mbie-accredited-employers.xlsx",
      "original/unrecognised-release.xlsx",
    ];
    ...
    try {
      ...
      await env.OPEN_DATA_BUCKET.put(
        "original/2025-06-02/List of Accredited Employers as at 2 June 2025.xlsx",
        "2025-06-02 fixture",
      );
      await env.OPEN_DATA_BUCKET.put(
      ...
      const firstCatalog = await env.OPEN_DATA_BUCKET.get("catalog.json");
      const first = await firstCatalog?.json<{
        originals: Array<{
      ...
      expect(first?.originals).toHaveLength(2);
      expect(first?.originals[0]).toMatchObject({
      ...
      expect(first?.originals[1]).toMatchObject({
        snapshot_date: "2025-06-02",
        row_count: 24_202,
        nzbn_count: 0,
        sha256: "c59d013579632121e200116a1f2a7784f62f962d7bab667e5ce62e235bcf842b",
        download_path: "/original/2025-06-02/List of Accredited Employers as at 2 June 2025.xlsx",
      });

      await env.OPEN_DATA_BUCKET.put("original/unrecognised-release.xlsx", "future fixture");
      await worker.scheduled(
        {
      ...
      const secondCatalog = await env.OPEN_DATA_BUCKET.get("catalog.json");
      const secondData = await secondCatalog?.json<{
        originals: Array<{
          snapshot_date: string | null;
          title: string;
          row_count: number | null;
      ...
      expect(secondData?.originals).toHaveLength(3);
      const unknown = secondData?.originals.find(
        (entry) => entry.download_path === "/original/unrecognised-release.xlsx",
      );
      expect(unknown).toMatchObject({
        snapshot_date: null,
        title: "unrecognised-release",
        row_count: null,
        sha256: null,
      });
      expect(secondData?.snapshots).toEqual(first?.snapshots);
    } finally {
      await env.OPEN_DATA_BUCKET.delete(keys);
    }
  });
