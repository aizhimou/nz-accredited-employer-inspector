...
the current Auckland date plus three calendar days. It queries INZ sequentially, with at least
...
```sh
cp scripts/employer-refresh/.env.example scripts/employer-refresh/.env.local
```
...
```sh
node --env-file=scripts/employer-refresh/.env.local \
  scripts/employer-refresh/refresh-expiring-employers.mjs
```
...
Positive results replace the latest INZ name, trading name, and expiry fields when the fetched
expiry date differs from the stored one. When the expiry date is unchanged, only the verification
and refresh-control metadata is updated, leaving the employer fields as they were. A recognized INZ
...
```sh
node --test scripts/employer-refresh/refresh-expiring-employers.test.mjs
```
